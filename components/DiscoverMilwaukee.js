...
const PAGE_SEO = {
  home: {
    title: "Discover Milwaukee | Your Insider's Guide to Milwaukee",
    description: "The hidden gems. The local favorites. The spots only real Milwaukeeans know about. Your weekly guide to everything Milwaukee.",
    keywords: "Milwaukee, Wisconsin, local guide, things to do Milwaukee, Milwaukee events, Milwaukee restaurants, Milwaukee bars"
  },
  explore: {
    title: "Explore Milwaukee | Guides, Lists & Local Favorites",
    description: "Discover the best of Milwaukee with our curated guides, neighborhood spotlights, and insider recommendations for food, drinks, and activities.",
    keywords: "Milwaukee guides, Milwaukee neighborhoods, best restaurants Milwaukee, things to do Milwaukee, Milwaukee travel guide"
  },
  events: {
    title: "Milwaukee Events Calendar | What's Happening This Week",
    description: "Find the best events in Milwaukee this week. Concerts, festivals, food & drink events, sports, comedy, and more. Updated weekly.",
    keywords: "Milwaukee events, things to do Milwaukee this weekend, Milwaukee concerts, Milwaukee festivals, Milwaukee calendar"
  },
  newsletter: {
    title: "Subscribe to Discover Milwaukee | Weekly Milwaukee Newsletter",
    description: "Get Milwaukee's best events, new restaurants, and local favorites delivered to your inbox every Wednesday. Join 10,000+ Milwaukee insiders.",
    keywords: "Milwaukee newsletter, Milwaukee email, Milwaukee weekly guide, subscribe Milwaukee"
  },
  partner: {
    title: "Partner With Discover Milwaukee | Advertising & Sponsorship",
    description: "Reach 22+ million engaged Milwaukee locals. Partner with Discover Milwaukee for sponsored content, event promotion, and brand partnerships.",
    keywords: "Milwaukee advertising, Milwaukee sponsorship, Milwaukee marketing, partner Discover Milwaukee, Milwaukee influencer"
  }
};
...
export function DiscoverMilwaukee({ initialPage = "home" }) {
  ...
  const handleSubscribe = async (e) => {
    ...
    try {
      ...
      const response = await fetch("https://discover-milwaukee-weekly.beehiiv.com/api/v1/subscriptions/subscribe", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          email: email,
          publication_id: "discover-milwaukee-weekly"
        })
      });
      
      // Beehiiv might block CORS, so we use their form action as fallback
      if (!response.ok) {
        // Use iframe method as fallback
        const iframe = document.createElement('iframe');
        iframe.style.display = 'none';
        iframe.name = 'beehiiv-submit';
        document.body.appendChild(iframe);
        
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = `https://discover-milwaukee-weekly.beehiiv.com/subscribe`;
        form.target = 'beehiiv-submit';
      ...
      // Fallback: Use iframe submission (works even with CORS)
      const iframe = document.createElement('iframe');
      iframe.style.display = 'none';
      iframe.name = 'beehiiv-submit';
      document.body.appendChild(iframe);
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: c.cream, fontFamily: "system-ui, sans-serif" }}>
      ...
      <Head>
        ...
        {/* Canonical URL */}
        <link rel="canonical" href={`https://discover-milwaukee.com${PAGE_ROUTES[page] || "/"}`} />
      </Head>
